...
if($_GET['id'] == 'login'){
    ...
    if($hitung > 0)
    {
        ...
        {
            echo '<script>alert("✔️ Login Sukses!");window.location="../admin/index.php";</script>';    
        }
        else
        {
            echo '<script>alert("✔️ Login Sukses!");window.location="../index.php";</script>'; 
        }
...
if($_GET['id'] == 'daftar')
{
    ...
    $row = $koneksi->prepare("SELECT * FROM user WHERE username = ?");
    ...
    if($hitung > 0)
    {
        echo '<script>alert("❌ Pendaftaran Gagal! Username sudah digunakan.");window.location="../index.php";</script>'; 
    }
    else
    {

        $sql = "INSERT INTO `user`(`nama_pengguna`, `username`, `password`, `level`)
                VALUES (?,?,?,?)";
...
if($_GET['id'] == 'booking')
{
    ...
    echo '<script>alert("Data Booking berhasil terkirim. Silakan melakukan pembayaran.");window.location="../bayar.php?id='.time().'";</script>'; 
}
...
if($_GET['id'] == 'konfirmasi')
{
    ...
    echo '<script>alert("✔️ Konfirmasi Sukses! Pembayaran Anda sedang diproses.");history.go(-2);</script>'; 
}
?>
